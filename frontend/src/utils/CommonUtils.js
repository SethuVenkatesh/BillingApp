...
export const invoiceValidation = Yup.object().shape({
  ...
  cgstPrice: Yup.string().matches(/^(?:[0-9]\d{0,7})(?:\.\d{2})?$/,
    "CGST price must be between 0.00 and 99999999.99 with two decimal places").required("CGST Price is required"),
  sgstPrice: Yup.string().matches(/^(?:[0-9]\d{0,7})(?:\.\d{2})?$/,
    "SGST price must be between 0.00 and 99999999.99 with two decimal places").required("SGST Price is required"),
  cgstPer: Yup.string().matches(/^(?:[0-3]\d{0,1})(?:\.\d{2})?$/,
    "CGST Percentage must be between 0.00 and 30.00 with two decimal places").required("CGST Percentage is required"),
  sgstPer: Yup.string().matches(/^(?:[0-3]\d{0,1})(?:\.\d{2})?$/,
    "SGST Percentage must be between 0.00 and 30.00 with two decimal places").required("SGST Percentage is required"),
