...
import { billingItemValidation } from '../../utils/CommonUtils'
...
let today = new Date();
  ...
  const [paymentInDetails,setPaymentInDetails]  = useState({
    items:[],
    subTotal:"",
    totalPrice:"",
    SGSTPrice:"",
  ...
  const addNewItems = () =>{
    ...
    const calculateBill = () =>{
        billingItemValidation.validate(addedItems,{abortEarly:false}).then((valid)=>{
            ...
            setPaymentInDetails({...paymentInDetails,subtotalPrice:totalPrice,invoiceItems:addedItems,totalPrice: GSTPrice,cgstPrice:CGSTPrice,sgstPrice:SGSTPrice,cgstPer:CGSTPer,sgstPer:SGSTPer,includeGst:includeGST})
        }).catch(error=>{
            setLoading(false);
            setToastMsg(error.inner[0]?.message);
            setToastStatus(false);  
    ...
    const handleCheckOut = () =>{
        ...
        setLoading(true);
        AuthorizedApi.patch("/invoice/update?invoiceNumber="+invoiceDetail.invoiceNumber,{...invoiceData}).then((res)=>{
            setToastStatus(true);
            setToastMsg(res.data.message);
            let invoiceDetail = res.data.data;
            setPaymentInDetails(invoiceDetail)
            setShowGST(invoiceDetail.includeGst)
            setGSTDetails({CGST:invoiceDetail.cgstPer,SGST:invoiceDetail.sgstPer})
            setCompanyDetails(invoiceDetail.invoiceParty)
            setPartyName(invoiceDetail.invoiceParty.partyName)
            setAddedItems(invoiceDetail.invoiceItems)
            setLoading(false);
        }).catch(err=>{
            console.log(err)
            setLoading(false);
    ...
    useEffect(()=>{
        let {invoiceDetail} = state;
        console.log("Invoice Details useeffectx`",state)
        setPaymentInDetails(invoiceDetail)
        setShowGST(invoiceDetail.includeGst)
        setGSTDetails({CGST:invoiceDetail.cgstPer,SGST:invoiceDetail.sgstPer})
        setCompanyDetails(invoiceDetail.invoiceParty)
    ...
    console.log("Invoice Details",state)
            ...
            loading ? <Loader/> : (
                <div className='p-4 m-2 border'>
                    ...
                    <div className='w-max ml-auto'>
                        <InputComponent inputType="date" labelName="Invoice Date" inputName="invoiceDate" inputValue={paymentInDetails.invoiceDate} jsonDetails={paymentInDetails} setJsonDetails={setPaymentInDetails} maxValue={maxDate}/>
                    </div>
                    <div className='border-gray-300 border-b mb-4 pb-4'>
                        ...
                        <div className='grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-x-4 gap-y-4'>   
                            ...
                            <InputComponent inputType="text" labelName="mobile number" inputName="mobileNumber" inputValue={companyDetails.mobileNumber} jsonDetails={companyDetails} setJsonDetails={setCompanyDetails}/>                        
                            <InputComponent inputType="text" labelName="alternate mobile number" inputName="altMobileNumber" inputValue={companyDetails.altMobileNumber} jsonDetails={companyDetails} setJsonDetails={setCompanyDetails}/>                        
                        </div>
                    </div>
                    <div className=''>
                        <div className='grid lg:grid-cols-2 md:grid-cols-1 '>
                            <div>
                                ...
                                {
                                    showGST && 
                                    <div className='flex gap-x-2 gap-y-4 mb-4'>
